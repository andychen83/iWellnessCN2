...
public class UtilTooth {
	...
	public static String lbozToString(double lboz){
		String text = UtilTooth.lbToLBOZ_F(lboz)+"";
...
//			text = wg.substring(0, wg.indexOf(":"))+"磅:"+wg.substring(wg.indexOf(":")+1,wg.length())+"盎司";
	...
	public static String gToMl2(float tempKg){
		BigDecimal b0 = new BigDecimal(String.valueOf(tempKg));
		BigDecimal b1 = new BigDecimal("63701");
		BigDecimal b4 = new BigDecimal("65536");
		BigDecimal b3 = new BigDecimal(String.valueOf(b0.multiply(b1).doubleValue())).divide(b4, 2, BigDecimal.ROUND_HALF_UP);

		return String.valueOf(b3);
	}

	public static float gTogml(float g) {
		float kg = (float) (g * 0.97);
		BigDecimal b = new BigDecimal(kg);
		float f1 = b.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();
		return f1;
	}
